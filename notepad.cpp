...
管理端
    2017-10-13 日与外包方确定的修改及需要完善内容
        考虑商家转账细节（有可能会设计到新增一套商家转账页面）
        新增商家账单记录页面
        设置商家管理人员权限（商家法人、商家店员、商家管理员、玩家）
            商家法人拥有最高权限，一旦创建不能更换，不能降级
            商家管理员也拥有最高权限，管理员可以创建多个，可以更换、降级
            商家店员只有部分权限
            一旦将商家店员和商家管理员降级为玩家，那么他将失去进入商家端的权限
            目前这三者角色在商家端是没有权限区别的，目的是实现多个人管理一个商家的目的。
        转账途径为直达商家，则不能设置分成比例
        设置商家是否按照设备分成
        修改商家名称输入框提示文字
        去掉自定义列表项
        ...
        增加设备管理大项，先根据产品创建设备，再将设备分配给店面
        商家名称创建后就不能修改
        不确定微信能否自动转账出去
        如果玩家被冻结，在进入游戏中心的时候，显示冻结提示静态页面
        去掉玩家二维码列
        去掉玩家跳转商家
        充值》输入钱数》扣费》三方扣费成功》点券到账》钱到账》商家到账
        去掉所有的按账期选择框
        所有列表页都做翻页
        补充店面文本框提示语
        关键信息就该加提示文字，并需要最高权限
...
计算运营定价【2017-10-13】
    ...
    标准版
        成本
            ...
            安装售后成本
        定价
            首付款模式 总价 48800 首付款 23000 ~ 25000 分成比例？ 单次价格？
